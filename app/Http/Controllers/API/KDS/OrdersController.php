...
class OrdersController extends Controller
{
    ...
    private function orderToCartModel($order)
    {
        ...
        return [
            ...
            'isFromDbOrder' => true,
            'expeditionType' => $order->getExpeditionType() ? $order->getExpeditionType() : '',
            'time_slot' => $order->time_formated ? $order->time_formated : '',
     ...
     */
    public function index($finished = 'false'): JsonResponse
    {
        ...
        if ($finished == 'false') {
            usort($orders, [$this, 'cmpBySort']);
        } else {
            usort($orders, [$this, 'cmpBySortRev']);
        }
